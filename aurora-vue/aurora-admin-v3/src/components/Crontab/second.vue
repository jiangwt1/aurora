<template>
  <n-form size="small">
    <n-form-item>
      <n-radio v-model:checked="radioValue" :value="1"> 秒，允许的通配符[, - * /] </n-radio>
    </n-form-item>
    <n-form-item>
      <n-radio v-model:checked="radioValue" :value="2">
        周期从
        <n-input-number v-model:value="cycle01" :min="0" :max="58" size="small" style="width: 80px" /> -
        <n-input-number v-model:value="cycle02" :min="cycle01 ? cycle01 + 1 : 1" :max="59" size="small" style="width: 80px" /> 秒
      </n-radio>
    </n-form-item>
    <n-form-item>
      <n-radio v-model:checked="radioValue" :value="3">
        从
        <n-input-number v-model:value="average01" :min="0" :max="58" size="small" style="width: 80px" /> 秒开始，每
        <n-input-number v-model:value="average02" :min="1" :max="59 - average01 || 0" size="small" style="width: 80px" /> 秒执行一次
      </n-radio>
    </n-form-item>
    <n-form-item>
