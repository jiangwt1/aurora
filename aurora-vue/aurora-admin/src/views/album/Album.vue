...
export default {
  ...
  methods: {
    ...
    addOrEditAlbum() {
      if (!this.albumForum.albumName || this.albumForum.albumName.trim() == '') {
        this.$message.error('相册名称不能为空')
        return false
      }
      if (!this.albumForum.albumDesc || this.albumForum.albumDesc.trim() == '') {
    ...
    uploadError(err, file, fileList) {
      // el-upload的on-error回调，err是错误对象
      // 尝试解析错误信息
      let errorMessage = '文件上传失败，请检查系统配置'
